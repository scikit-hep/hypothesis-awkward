...
- `docs/guide/`: Overview, Installation, Getting Started — thin (install + a
  first use) — a Roadmap (state-of-the-project / meta), and two explanation
  pages, Testing Awkward Array and Generating and Shrinking Samples. No how-to
  layer yet.
...
container of single-purpose sections, and unmarked content is out of quadrant by
construction. Before reporting, each persona self-checks alignment and routes
any out-of-scope ask or on-page out-of-quadrant content to the page that owns
that quadrant, rather than reporting it as a defect of this page. A page is
written well when the personas it serves find it relevant and useful, and the
personas it does not serve can tell early in the page that it is not for them,
while still seeing that it is useful to other readers. See
[`2026-06-17-01-README-intro-persona-review.md`](./2026-06-17-01-README-intro-persona-review.md).
...
Carried over from the old page plan: `docs/index.md` still duplicates the README
intro and needs the sync described under **Current state**.
