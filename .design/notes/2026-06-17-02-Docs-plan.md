...
is a fixed lens that is told the page's declared Diátaxis quadrant(s) and asked
the matching reader question — for explanation, for example, "after reading, do
you understand it?" — and reviews in that mode via
[`.claude/rules/diataxis-review.md`](../../.claude/rules/diataxis-review.md)
through its lens (the four quadrant questions live there). A page may be
declared with more than one quadrant, and is then asked each question;
undeclared cross-quadrant content is out of quadrant. Before reporting, each
...
| Getting Started                          | Tut                | Downstream dev / Researcher              | on API change       | Exists (rewritten 2026-07)          |
| API Reference                            | Ref                | All                                      | auto (docstrings)   | Exists (generated)                  |
| Testing Awkward Array (incl. bugs found) | Expl + living log  | Awkward core dev / Evaluator             | per release         | Exists (in Guide)                   |
| Roadmap                                  | Expl/meta (living) | Awkward core dev / Downstream dev        | per milestone       | Exists (in Guide)                   |
| How samples are generated & shrink       | Expl               | Hypothesis-PBT expert / Awkward core dev | on algorithm change | Exists (in Guide)                   |
