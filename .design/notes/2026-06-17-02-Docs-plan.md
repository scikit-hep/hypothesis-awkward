...
[`2026-06-17-01-README-intro-persona-review.md`](./2026-06-17-01-README-intro-persona-review.md).
Each persona is implemented as a `.claude/agents/docs-persona-*` subagent that
the `write-docs-page` skill launches in parallel; the table below is the design
rationale.
...
- Persona detail (the persona table, persona prose) lives here → extract it to
  the `.claude/agents/docs-persona-*` files and leave only a pointer, so this
  note stays agnostic to personas and their details remain an agent
