...
- `docs/guide/`: Installation, Getting Started — thin (install + a first use) —
  a Roadmap (state-of-the-project / meta), and two explanation pages, Testing
  Awkward Array and Generating and Shrinking Samples; the overview is
  `docs/index.md`. Installation is the only how-to unit; there is no
  task-oriented how-to layer yet.
...
- **Symptom to fix**: the README intro and
  [`docs/index.md`](../../docs/index.md) began as near-verbatim copies; the
  README was rewritten in 2026-06 and `docs/index.md` kept the old text, so the
  two have diverged. Hand-written prose goes out of date and gets duplicated,
  while the generated reference never does. The fix is one source and a link,
  not a resync. See
...
synthesize or select → re-review the page until it ships** (accuracy beats
style). This loop is operationalized as the
[`write-docs-page`](../../.claude/skills/write-docs-page/SKILL.md) skill, with
operational conventions (build, fences, nav) in
[`.claude/rules/docs.md`](../../.claude/rules/docs.md). The docs use six
personas that represent the target readers (below). They refine the four from
the README review, drop Docs Editor into the editorial rubric (see **Voice**),
and add the downstream package developer, the evaluator, and AI. Every persona
reviews every page, and each also rates how relevant the page is to them. Each
is a fixed lens that is told the declared Diátaxis quadrant of each unit of
content and asked the matching reader question — for explanation, for example,
"after reading, do you understand it?" — and reviews in that mode via
[`diataxis-review.md`](https://github.com/TaiSakuma/reviewed-writer/blob/main/skills/persona-review/references/diataxis-review.md)
through its lens (the four quadrant questions live there). Each unit declares
one quadrant with a marker in the page itself: a page is either one unit or a
container of single-purpose sections, and content belonging to no unit is out of
quadrant, apart from a container's orientation preamble. Before reporting, each
persona self-checks alignment and routes any out-of-scope ask or on-page
out-of-quadrant content to the page that owns that quadrant, rather than
reporting it as a defect of this page. A page is written well when the personas
it serves find it relevant and useful, and the personas it does not serve can
tell early in the page that it is not for them, while still seeing that it is
useful to other readers. See
[`2026-06-17-01-README-intro-persona-review.md`](./2026-06-17-01-README-intro-persona-review.md).
Each persona is a head file in `.claude/personas/` that the `reviewed-writer`
plugin's `persona-review` skill (invoked by `write-docs-page`) hands to its
generic reviewer subagent, launched in parallel; the table below is the design
rationale.
...
adapted from `voice.md` in the presentation repository. The rules now live in
the path-scoped
[`.claude/rules/docs-voice.md`](../../.claude/rules/docs-voice.md), scoped to
the README and `docs/` (not code, `.design/` notes, or commit and PR text).
...
Pages not written yet. The pages that exist are in
[`.design/docs/Chapters.md`](../docs/Chapters.md), which carries their primary
audience and update trigger; their quadrants are declared by markers in the
pages themselves. A row moves from here to that table when the page ships.

`Q` is the quadrant the page is _intended_ to occupy — planning intent, not a
declaration, and first-pass. "Primary audience" gives short forms of the persona
names — who the page mainly serves; all personas review every page.

| Page                           | Q                 | Primary audience                   | Update trigger     |
| ------------------------------ | ----------------- | ---------------------------------- | ------------------ |
| How-to cookbook (see below)    | How-to            | Downstream dev / Researcher        | on API change      |
| Glossary                       | Ref/Expl          | All (bridges audiences)            | rare               |
| Citation ("Cite this")         | meta              | Evaluator                          | on release/DOI     |
| Development methodology        | Expl/contributing | Awkward core dev / Downstream dev  | rare               |
| Audience bridges (2 primers)   | Expl              | Researcher / Hypothesis-PBT expert | rare               |
| Supported dtypes/layouts table | Ref               | Awkward core dev / Downstream dev  | auto (from source) |
| FAQ / troubleshooting          | How-to            | Researcher / Downstream dev        | as issues arise    |

A page tagged with two quadrants above is a plan to write two single-purpose
sections, each with its own marker — not a mixed unit.

Carried over from the old page plan: `docs/index.md` has diverged from the
README intro and needs the treatment described under **Current state**.
...
**Routed items.** Where a review sends an out-of-scope ask, or content whose
destination page does not exist yet. From the guide-page persona re-review
(2026-07-03):

- Roadmap: state the fate of `constructors.arrays()` once the reserved bare
  `st_ak.arrays()` lands (kept, wrapped, or deprecated) — needs a maintainer
  decision; no design note records one yet.
- API reference: add reference pages for the `types` and `forms` strategy
  modules, so `st_ak.numpy_types()` and `st_ak.numpy_forms()` (named on the
  Roadmap) become linkable.
- How-to cookbook: a worked property test on the reader's own analysis code —
  the target for Getting Started's "a real test asserts a property of your own
  code" sentence once it exists.

From the _Testing Awkward Array_ revision (2026-08-07):

- How-to cookbook: run the strategies in a reader's own CI the way Awkward does
  (a nightly Hypothesis settings profile with a large `max_examples` and
  statistics); write known-issue predicates for one's own suite; reproduce a CI
  failure locally (`HYPOTHESIS_PROFILE`, replaying the `@reproduce_failure`
  blob).
- API reference: a per-module coverage table of the upstream property tests
  (module → operation → strategy restrictions → guarded issues).
- Glossary: entries for "draw", "example", and "oracle".
- Roadmap: a fuller boundary statement for `arrays()` (layout parameters,
  behaviors, backends) beyond the one-clause boundary the page now carries.
- Project-level: a citation path (`CITATION.cff` plus a "how to cite" line on
  `docs/index.md`).
- Docs: a minimum-Hypothesis-version note (`hypothesis>=6.152.4`, from the #4708
  fix) — verify the floor's motivation before stating the link.
- Cross-page accuracy report (needs its own update run):
  `generating-and-shrinking-samples.md` states `leaf_contents()` shrinks toward
  `EmptyArray`, then bytestring, string, and `NumpyArray`; since PRs #170/#173
  the source order has the zero-field `RecordArray` in second position.
...
- Per-page "last updated" date — defer to Zensical revisioning when it ships
  (`git-revision-date-localized` is Tier 2,
  [Backlog #18](https://github.com/zensical/backlog/issues/18)); rely on the
  `mike` version meanwhile.
...
- Exemplars section grows or more sites are added → promote to
  ...
  bucket).
- Docs-initiative notes exceed ~4 → move them into `.design/docs/`, which now
  holds the chapters table.
- Persona detail (the persona table, persona prose) lives here → extract it to
  the `.claude/personas/` files and leave only a pointer, so this note stays
  agnostic to personas and their details remain an implementation detail.

## Related

- [`2026-06-17-01-README-intro-persona-review.md`](./2026-06-17-01-README-intro-persona-review.md)
  — first worked example of the narrative-track workflow.
- [`.design/docs/Chapters.md`](../docs/Chapters.md) — the pages that exist, with
  their primary audience and update trigger.
- [`.claude/skills/write-docs-page/`](../../.claude/skills/write-docs-page/SKILL.md)
  — the narrative-track workflow as a skill.
- [`.claude/rules/docs-voice.md`](../../.claude/rules/docs-voice.md) — voice
  rules (extracted from **Voice** above).
- [`.claude/rules/docs.md`](../../.claude/rules/docs.md) — operational docs
  conventions (build, fences, nav, cross-linking).
