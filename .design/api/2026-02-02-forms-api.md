...
From the [NumpyForm research](./../research/2026-02-02-numpy-form-research.md)
and [types API design](./../api/2026-01-21-types-api.md):
...
2. **Control over `inner_shape`**: The key feature distinguishing forms from
   types
...
When `type_` is given, the form is fully determined: the primitive is copied
from the type's `primitive` attribute and `inner_shape` is forced to `()`. This
is because any non-empty `inner_shape` would make `NumpyForm.type` return
...
**Decision:** Accept an optional `type_` parameter that generates a form
matching a given `NumpyType`.
...
- For `NumpyType` the mapping is 1:1 (primitive + empty inner_shape), but the
  pattern becomes valuable for higher-level forms where one type maps to
...
- `type_` and `dtypes` are mutually exclusive in intent; `type_` takes
  precedence
...
**Decision:** Accept
`tuple[int, ...] | st.SearchStrategy[tuple[int, ...]] | None`.
