...
From the [UX research](./../research/2026-01-21-02-ux-interview-01.md) and
[type system research](./../research/2026-01-21-01-type-system-research.md):
...
**Decision:** `types()` generates content types, not `ArrayType` or
`ScalarType`.
...
1. **Should we support parameters?** Types can have arbitrary parameters (e.g.,
   `__record__` for named records). Should we generate these?
...
4. **Minimum union variants?** UnionType requires at least 2 variants. Should we
   expose `min_union_variants`?
...
[arrays-api.md](./2026-02-04-01-arrays-api.md)). The types strategies remain
