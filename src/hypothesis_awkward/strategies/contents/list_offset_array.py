...
def list_offset_array_contents(
    ...
    *,
    ...
    >>> c = list_offset_array_contents(min_length=2, max_length=4).example()
    >>> 2 <= len(c) <= 4
    True
    """
    match content:
    ...
    offsets_list = draw(
        _st_offsets(content_len, min_length=min_length, max_length=max_length)
    )
...
def _st_offsets_no_unreachable(
    ...
    *,
    ...
    if content_len == 0:
        ...
        return draw(st.lists(st.just(0), min_size=min_length + 1, max_size=ml))
...
def list_offset_array_from_contents(
    ...
    *,
    ...
    return draw(
        list_offset_array_contents(st_content, min_length=length, max_length=length)
    )
